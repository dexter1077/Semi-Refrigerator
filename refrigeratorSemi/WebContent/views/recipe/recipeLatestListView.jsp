...
<body>
    ...
    <%@include file="../common/user/menubar.jsp" %>
        ...
        <div align="center" class="paging-area">
			...
			<% if(currentPage != 1) { %>
				<button onclick="location.href='<%= contextPath%>/latestList.recipe?currentPage=<%=currentPage-1%>';"> &lt; </button>
			<% } %>
						
				<% for(int p=startPage; p<=endPage; p++) {%>
			            	
					<% if(p != currentPage){ %>
						<button onclick="location.href='<%= contextPath%>/latestList.recipe?currentPage=<%=p%>';"><%= p %></button>
					<% }else{ %>
